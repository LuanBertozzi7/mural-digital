...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    setError(null)
    try {
      const { token, user } = await apiFetch('/api/auth/login', { method: 'POST', body: JSON.stringify(form) })
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-50">Bem-vindo de volta</h1>
        ...
        <p className="text-sm text-gray-400 text-center mt-5">
          Não tem conta?{' '}
          <Link to="/register" className="text-blue-600 font-medium hover:underline">Cadastrar</Link>
        </p>
      </div>
    </div>
  )
